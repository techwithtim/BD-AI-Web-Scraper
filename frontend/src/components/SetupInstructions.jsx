...
const SetupInstructions = ({ instructions }) => {
    ...
    const renderContent = (content) => {
        ...
        return parts.map((part, index) => {
            if (index % 2 === 1) {
                ...
                return <CodeDisplay key={index} code={code.join('\n')} language={language.trim()} />;
            } else {
                    ...
                    <Paragraph key={index}>
                        {part.split('\n').map((line, lineIndex) => {
                            if (line.startsWith('- [')) {
                                ...
                                return <div key={lineIndex}><Link href={url} target="_blank">{text}</Link></div>;
                            } else {
    ...
    return (
        <Card title={<Title level={4}>Setup Instructions</Title>} style={{ marginBottom: 16 }}>
